...
void rango(int C[10][11],int num, int numEle, int A[10], int B[10][10])
{
    ...
    if(ampliada == noampliada)
    {
        if(ampliada < nVariables){
            printf("\nii)El sistema tiene infinidad de soluciones. R(A*)=%d R(A)=%d n=%d, una de sus soluciones es:\n", ampliada, noampliada,nVariables);
            ...
            for (i = 0; i < num; i++)
	        {
	        	//printf("El escalar α%d es : %f\n",(i+1), (float)(C[i][10]) / (float)(C[i][i]) );
	        }
	        */
